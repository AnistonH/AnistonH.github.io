...
// 取三元组的任意一个分量
ElemType getComponent(Triplet* triplet, int index) {
...
// 设置三元组的任意一个分量
void setComponent(Triplet* triplet, int index, ElemType value) {
    switch (index) {
    case 1:
        triplet->first = value;
        break;
...
// 求三元组的最大分量
ElemType maxComponent(Triplet* triplet) {
...
// 求三元组的最小分量
ElemType minComponent(Triplet* triplet) {
...
int main() {
    ...
    int index;
    scanf("%d", &index);
    printf("对应的值是: %d\n", getComponent(myTriplet, index));

    printf("请输入要修改的索引号，以及值：");
    ElemType modify;
    scanf("%d %d", &index, &modify);
    setComponent(myTriplet, index, modify);
    printf("修改成功");
    displayTriplet(myTriplet);

    printf("三元组最大值是: %d\n", maxComponent(myTriplet));
    printf("三元组最小值是: %d\n", minComponent(myTriplet));
